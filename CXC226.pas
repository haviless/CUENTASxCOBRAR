...
Type
   TFENVIALETBCO = Class(TForm)
      ...
      lblNroPlla: TLabel;
      lblFEnvio: TLabel;
      dbdtpFEnvio: TwwDBDateTimePicker;
      dbeNroPlla: TwwDBEdit;
...
Procedure TFENVIALETBCO.dblcUBIChange(Sender: TObject);
Begin
   sUBI := dblcUBI.Text;
   dbeUBI.Text := DMCXC.cdsUbica.FieldByName('UBICADES').AsString;
End;

Procedure TFENVIALETBCO.dblcExist(Sender: TObject);
Var
   bRq: Boolean;
...
Procedure TFENVIALETBCO.dblcNotInList(Sender: TObject;
   LookupTable: TDataSet; NewValue: String; Var Accept: Boolean);
Begin
...
Procedure TFENVIALETBCO.Z2bbtnCancelClick(Sender: TObject);
Begin
   close;
End;

Procedure TFENVIALETBCO.dblcSITChange(Sender: TObject);
Begin
   sSIT := dblcSIT.Text;
   dbeSIT.Text := DMCXC.cdsSitua.FieldByName('SITUADES').AsString;
End;

Procedure TFENVIALETBCO.FormShow(Sender: TObject);
Begin
   DMCXC.cdsUbica.Open;
   DMCXC.cdsSitua.Open;
//  cdsX.Open;
...
Procedure TFENVIALETBCO.FormCreate(Sender: TObject);
Begin
//  cdsX.Close;
//  cdsX.CreateDataSet;
End;

Procedure TFENVIALETBCO.GetData;
{var
   i, f, j : Integer;}
Var
...
Procedure TFENVIALETBCO.Z2bbtnAceptarClick(Sender: TObject);
Var
...
Procedure TFENVIALETBCO.ppHeaderBand1BeforePrint(Sender: TObject);
Var
   sCIA: String;
Begin
   sCIA := DMCXC.cdsQry7.FieldByName('CIAID').AsString;
...
Procedure TFENVIALETBCO.FormKeyPress(Sender: TObject; Var Key: Char);
Begin
   If key = #13 Then
   Begin
      key := #0;
...
Procedure TFENVIALETBCO.Z2bbtnImprimirClick(Sender: TObject);
Begin
   pprCambio.print;
End;

Procedure TFENVIALETBCO.dblcUBIExit(Sender: TObject);
Begin
   If Length(dblcUBI.text) = 0 Then
   Begin
...
Procedure TFENVIALETBCO.dblcSITExit(Sender: TObject);
Begin
{  if Length(dblcSIT.text)=0 then
  begin
...
Procedure TFENVIALETBCO.Z2bbtnSumaClick(Sender: TObject);
Var
